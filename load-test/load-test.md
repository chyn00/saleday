...
## 성능 테스트 결과 (Tomcat Thread Pool, HikariCP, Warm-up 튜닝)

성능 테스트는 **단일 인스턴스 환경에서 수행**하였습니다.

동일 머신에서 멀티 인스턴스를 구성할 경우,  
CPU, 메모리, 커넥션 풀 등 자원을 공유하면서 인스턴스 간 간섭이 발생하고,  
이로 인해 **성능 병목의 원인이 왜곡될 수 있다고 판단**했습니다.

따라서, **병목을 명확히 식별하고 튜닝 효과를 검증하기 위해 단일 인스턴스 기준으로 측정**했습니다.

---

한편, **분산 환경에서의 동작 보장은 별도로 검증**했습니다.

멀티 인스턴스 환경에서도 아래 항목들이 정상적으로 동작함을 확인했습니다.

- Redis 원자 연산
- Kafka 메시지 처리
- DB 정합성 유지

즉, **성능 검증(단일 인스턴스)과 정합성 검증(멀티 인스턴스)을 분리하여 확인**했습니다.
